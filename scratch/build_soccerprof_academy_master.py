...
<head>
  ...
  <style>
    ...
    /* Signature Logo Background Particle Overlay */
    .logo-particle-bg {{
      position: absolute;
      inset: 0;
      pointer-events: none;
...
def get_scripts():
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Dynamic Logo Background Particle Overlay
      ...
      window.addEventListener('scroll', () => {
        ...
        particles.forEach((p, idx) => {
          const speed = (idx % 5 + 1) * 0.03;
          p.style.transform = `translateY(${scrolled * speed}px) rotate(${scrolled * 0.05 + idx * 10}deg)`;
        });
      });
    });
